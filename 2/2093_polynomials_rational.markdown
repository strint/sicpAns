...
(define (install-polynomial-package)
  ...
  (define (div-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((L (div-terms (term-list p1) (term-list p2)))
              (V (variable p1)))
          (list (tag (make-poly V (car L))) (tag (make-poly V (cadr L)))))
        (error "Polys not in same var -- MUL-POLY"
               (list p1 p2))))
        ...
        (let ((t1 (first-term L1))
          ...
          (if (> (order t2) (order t1))
              ...
              (let ((new-c (div (coeff t1) (coeff t2)))
                ...
                (let* ((quotient-p (make-term new-o new-c))
                       (remainder-p (neg-terms (rest-terms (mul-term-by-all-terms 
                                                             quotient-p
                                                             L2)))))
                  (let ((rest-of-result
                          (div-terms (add-terms remainder-p
                                                (rest-terms L1))
                                     L2)))
                    (list (add-terms (list quotient-p) (car rest-of-result))
                          (cadr rest-of-result)))))))))
  ;(trace-both div-terms)
...
;  (lambda (ra) (make-integer (round (exact->inexact (/ (numer ra)
;                                                       (denom ra)))))))
(define (project x)
...
(define t5 (list 0 (make-integer -1)))
(define t6 (list 5 (make-integer 1)))
(define t7 (list 2 (make-integer 1)))
(define tl5 (list t6 t5))
(define tl6 (list t7 t5))
(define p5 (make-polynomial 'x tl5))
(define p6 (make-polynomial 'x tl6))
